...
/**
 * FrontendException is a RuntimeException, because it shouldn't be caught (excepted in the rest
 * controllers)
 * it is used to give an own description for the exception which has to reach the frontend
 * if the rest controller catches the FrontendException, it has to send it as an attribute of the
 * response. So a
...
public final class FrontendException extends RuntimeException {
  ...
  public static FrontendException IO(Throwable e) {
    return exceptionOf(e, "IO Exception", "Something went wrong while reading or writing files on"
                                          + " the server.");
  }
  ...
  public static FrontendException USER_MANAGEMENT(Throwable e) {
    return exceptionOf(e, "User Management Exception", "Something went wrong during user "
                                                       + "management.");
  }

  public static FrontendException PYTHON_EXCEPTION(String traceback) {
    return exceptionOf(traceback, "Python Integration Exception", "Something went wrong inside "
                                                                  + "the python integration "
                                                                  + "server while running your "
                                                                  + "request.");
  }
