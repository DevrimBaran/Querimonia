...
class Tag extends Component {
  ...
  averageLuminance = (avg, entity, array) => {
    const color = (this.props.colors && this.props.colors[entity.label]
      ? this.props.colors[entity.label] : entity.color) || '#cccccc';
    ...
    return (
      b ? (<div ref={this.tooltip} className='tooltip'>
        {entities.map((label, i) => (
          <div key={i}>
            <span className='dot' style={{ marginRight: '0.4em',
              backgroundColor: (this.props.colors && this.props.colors[label.label]
                ? this.props.colors[label.label] : label.color) || '#cccccc' }} />
            {label.label}
          </div>
        ))}
      </div>)
          ...
          {entities.map((label, i) => (
            <div style={
              (i !== 0 ? { marginLeft: '0.4em',
                ...
                float: 'left',
                padding: '4px' }
                : { border: '2px solid ' + (this.props.colors && this.props.colors[label.label]
                  ? this.props.colors[label.label] : label.color) || '#cccccc',
                textAlign: 'center',
                float: 'left',
                padding: '4px' })
              ...
              <span className='action-button'>
                <i title='Kopieren' id='editEntity' className={'far fa-clone'} onClick={this.editEntity.bind(this, label, false)} />
              </span>
              <span className='action-button'>
                <i title='Bearbeiten' id='editEntity' className={'far fa-edit'} onClick={this.editEntity.bind(this, label, true)} />
              </span>
              <span className='action-button'>
...
const mapStateToProps = (state, props) => {
  ...
  console.log('state');
  console.log(state);
  if (state.currentConfig.extractors) {
    state.currentConfig.extractors.reduce((obj, extractor) => {
      if (extractor.color) {
        labels[extractor.label] = extractor.color;
      };
      colors = labels;
      return labels;
    }, {});
