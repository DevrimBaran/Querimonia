...
class OpenApi extends Component {
    ...
    checkOk = (response) => {
        ...
        throw "Response not okay!"; //TODO: Expected an object to be thrown  no-throw-literal
    }
    checkUpdate = () => {
    ...
    generateResponse = (resource, options) => {
        ...
        const route = getRoute(resource, options.mode)
        ...
        if ((process.env.NODE_ENV === 'development' || process.env.REACT_APP_BACKEND_PATH === 'mock')) {
            return (
                <div {...this.props}>
                    ...
                    <input id="openapi-active" type="checkbox" onClick={this.toggle} checked={this.state.active} />
